...
import {DecentralizedStableCoin} from "./DecentralizedStableCoins.sol";
...
contract DSCEngine is ReentrancyGuard {
    ...
    error DSCEngine_TokenAddAndPriceFeedAddMustBeSameLength();
    error DSCEngine_NotAllowedToken();
    error DSCEngine_TransferFailed();
    error DSCEngine_BreaksHealthFactor(uint256 healthFactor);
    error DSCEngine_MintFailed();
    error DSCEmgime_HealthFactorOK();
    ...
    constructor(address[] memory tokenAddresses, address[] memory priceFeedAddresses, address dscAddress) {
        if (tokenAddresses.length != priceFeedAddresses.length) {
            revert DSCEngine_TokenAddAndPriceFeedAddMustBeSameLength();
        }
    ...
    * @notice: A known bug would be if the protocol was only 100% collateralized, we wouldn't be able to liquidate
        ...
        nonReentrant
    {
        ...
        if (startingUserHealthFactor >= MIN_HEALTH_FACTOR) {
            revert DSCEmgime_HealthFactorOK();
        }
